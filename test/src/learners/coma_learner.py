...
class COMALearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        bs = batch.batch_size #batch size
        max_t = batch.max_seq_length
        rewards = batch["reward"][:, :-1]  #size (1, 25, 1)
        actions = batch["actions"][:, :]  #size (1, 26, 3, 1)
       
        #imitate_rewards = batch["imitate_reward"][:, :-1]


        terminated = batch["terminated"][:, :-1].float()
        mask = batch["filled"][:, :-1].float() #75
        ...
        # q_vals.shape == ([1, 25 ,3, 9])
        actions = actions[:,:-1] #([1, 25, 3, 1])
        ...
        # Calculated baseline
        q_vals = q_vals.reshape(-1, self.n_actions) #(72, 9)
        
        pi = mac_out.view(-1, self.n_actions)  #(72, 9)
        baseline = (pi * q_vals).sum(-1).detach() #(72, 9)
