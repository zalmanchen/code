...
class StarCraft2Env(MultiAgentEnv):
    ...
    def get_obs_agent(self, agent_id):
        ...
        self.enemy_state_raw = [move_feats, enemy_feats, ally_feats, own_feats]
    ...
    def get_enemy_action(self, enemy_id, step_distance=0.5):
        if not self.enemy_state:
            # enemy state is None
            self.enemy_state = self.enemies
            return 1
        elif not self.agent_state:
            # agent state is None
            self.agent_state = self.agents
            return 1

        next_state = self.enemies[enemy_id]
        state = self.enemy_state[enemy_id]

        # movement greater than 0.5 is considered moving
        next_xyz = [next_state.pos.x, next_state.pos.y, next_state.pos.z]
        xyz = [state.pos.x, state.pos.y, state.pos.z]

        residual = [next_xyz[i] - xyz[i] for i in range(min(len(next_xyz), len(xyz)))]

        # default  attack
        action_select = 1

        if residual[0] < 0. and abs(residual[0]) > step_distance:
            # west
            action_select = 4
        if residual[0] > 0. and abs(residual[0]) > step_distance:
            # east
            action_select = 5
        if residual[1] > 0. and abs(residual[1]) > step_distance:
            # north
            action_select = 2
        if residual[1] < 0. and abs(residual[1]) > step_distance:
            # north
        ...
        # attack
        agent_dis = [9999. for i in range(self.n_enemies)]
        for a_id, a_unit in self.agents.items():
            if self.agent_state[a_id].health - self.agents[a_id].health > 0:
                # being attacked
                shoot_dis = math.hypot(abs(a_unit.pos.x - next_state.pos.x) , abs(a_unit.pos.y - next_state.pos.y))
                if shoot_dis <= 6:
                    agent_dis[a_id] = shoot_dis

        if enemy_id == self.n_enemies - 1:
            # update
            self.agent_state = self.agents.copy()
            self.enemy_state = self.enemies.copy()

        if min(agent_dis) < 999.:
            action_select = 6 + agent_dis.index(min(agent_dis))

        # do-nothing
        return action_select

    def get_enemy_actions(self):
        return [self.get_enemy_action(e_id) for e_id in range(self.n_enemies)]
    ...
    def get_enemy_by_id(self, agent_id):
        return self.enemies[agent_id]

    def get_anti_obs_enemy_feats_size(self):
        """ Returns the dimensions of the matrix containing enemy features.
        Size is n_enemies x n_features.
        """
        nf_en = 4 + self.unit_type_bits
    ...
    def get_anti_obs_ally_feats_size(self):
        """Returns the dimensions of the matrix containing ally features.
        Size is n_allies x n_features.
        """
        nf_al = 4 + self.unit_type_bits
    ...
    def get_enemy_unit_by_id(self, a_id):
        """Get unit by ID"""
    ...
    def get_enemy_avail_actions(self):
        ...
        avail_actions = []
        for agent_id in range(self.n_agents):
            avail_agent = self.get_avail_enemy_actions(agent_id)
            avail_actions.append(avail_agent)
        return avail_actions

    def get_anti_obs_agent(self, agent_id):
        unit = self.get_enemy_by_id(agent_id)

        move_feats_dim = self.get_obs_move_feats_size()
        enemy_feats_dim = self.get_anti_obs_enemy_feats_size()
        ally_feats_dim = self.get_anti_obs_ally_feats_size()
        own_feats_dim = self.get_obs_own_feats_size()
        ...
        if unit.health > 0:
            ...
            # Enemy states, actually, it is the state of allies from players' perspactive
            for e_id, e_unit in self.agents.items():
                ...
                if (
                    ...
                    if self.unit_type_bits > 0:
                        ...
                        enemy_feats[e_id, ind + type_id] = 1  # unit type
            # Ally features. Similarly, it is the enemies from player's perspectives
            al_ids = [
                al_id for al_id in range(self.n_enemies) if al_id != agent_id
            ]
            for i, al_id in enumerate(al_ids):
    ...
    def get_anti_obs(self):
        enemies_obs = [self.get_anti_obs_agent(i) for i in range(self.n_enemies)]
        return enemies_obs
